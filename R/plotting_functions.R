...
figure_under <- function(under_reporting_data, testing_data, iso_code_arg)
{
  ...
  plot(plot_dat_under_reporting$date, plot_dat_under_reporting$estimate, 
       ...
       las = 1,
       xlab = "Fechas", 
       ylab = "Symptomatic cases reported (%)",
       bty = "n",
       axes = FALSE,
       xlim = x_limits_both, 
...
figure_1_fun <- function(under_reporting_data, testing_data, iso_code_arg)
{
  ...
  # # # y_limits_testing_dat <- plot_dat_testing %>%
  # # #   dplyr::summarise(y_min = min(testing_data),
  # # #                    y_max = max(testing_data))  
...
figure_3_fun <- function()
{
  ...
  country_codes <- readr::read_csv("/home/lucas/Escritorio/covid_underreporting-master/data/country_codes.csv")
  
  serology_study_results <- readr::read_csv("/home/lucas/Escritorio/covid_underreporting-master/data/national serology estimates.csv") %>%
