...
getUnderReportingCaseDeathPopulationAndTestingData <- function()
{
  ...
  underReportingPath <- paste(path,"data/current_estimates_extracted_not_age_adjusted/",sep="")
...
getAdjustedRegionalCaseAndSerologyData <- function()
{
  ...
  serology_study_results <- readr::read_csv(paste(path,"data/regional serology estimates.csv",sep="")) %>%
    dplyr::rowwise() %>%
  ...
  regional_lookup_table <- readr::read_csv(paste(path,"data/regional_codes_look_up.csv",sep=""))
...
globalPrevalenceEstimates <- function()
{
  ...
  data_path <- paste(path,"data/current_estimates_extracted_not_age_adjusted/",sep="")
  files <- dir(path = data_path,
               pattern = "*.rds")
