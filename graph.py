...
for region in ["Arica y Parinacota", "Aysén"]:
    ...
    ax7.set_title('Subreporte '+ regiones[region][1])
    ax7.fill_between(FECHAS.values[:], df["upper"].values[:]*100, df["lower"].values[:]*100, alpha=0.2)
    ax7.plot (FECHAS.values[:], df["estimate"].values[:]*100, label = "Subreporte "+regiones[region][1])
    ax7.legend (loc = "best")
    ax7.set_ylim(0, 100)
    ax7.set_ylabel("Porcentaje de casos sintomáticos reportados (%)")
    fig.savefig(os.path.join(out_folder,"subreporte_" + regiones[region][1] + ".png"), format='png', bbox_inches = 'tight')