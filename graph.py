...
FECHAS = np.array([pd.to_datetime (FECHA_INICIAL) + datetime.timedelta(days=i) for i in range(df.shape[0])])
FECHAS = pd.to_datetime(FECHAS)
fig = plt.figure('Subreporte Chile',figsize = (10, 5))
ax7 = fig.add_subplot(111)
ax7.set_title('Subreporte vs Tiempo')
# ax7.plot (FECHAS.values[:], df["lower"].values[:], label = "lower")
# ax7.plot (FECHAS.values[:], df["upper"].values[:], label = "Subreporte")
ax7.fill_between(FECHAS.values[:], df["upper"].values[:], df["lower"].values[:], alpha=0.2)
ax7.plot (FECHAS.values[:], df["estimate"].values[:], label = "Subreporte")
ax7.legend (loc = "best")
ax7.set_ylim(0, 1)
fig.savefig(os.path.join(out_folder,'underreport_Chile.png'), format='png', bbox_inches = 'tight')